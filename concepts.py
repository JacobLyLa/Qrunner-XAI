...
class Concept:
    def __init__(self, name, presence_predicate, max_size=500):
        self.name = name
        ...
        self.directory = f"concepts/{name}"
    ...
    def save_observations(self):
        ...
        prepare_folders(self.directory + "/presence_images")
        prepare_folders(self.directory + "/absence_images")
        ...
        for i, image in enumerate(self.presence_images):
            ...
            image.save(f"{self.directory}/presence_images/image_{i}.png")

        for i, image in enumerate(self.absence_images):
            image = Image.fromarray(image)
            image.save(f"{self.directory}/absence_images/image_{i}.png")

        # save concept observations as a single numpy array
        print("saving observations...")
        np.save(f"{self.directory}/presence_observations.npy", np.array(self.presence_observations))
        np.save(f"{self.directory}/absence_observations.npy", np.array(self.absence_observations))

    def save_cavs(self, cavs, layers):
        prepare_folders(f"{self.directory}/cavs")
        for layer, cav in zip(layers, cavs):
            np.save(f"{self.directory}/cavs/cav_{layer}.npy", cav)

    def load_cav(self, layer):
        return np.load(f"{self.directory}/cavs/cav_{layer}.npy")

    def get_presence_observations(self):
        return np.load(f"{self.directory}/presence_observations.npy")
        
    def get_absence_observations(self):
        return np.load(f"{self.directory}/absence_observations.npy")

    def get_presence_images(self):
        num_images = len(os.listdir(f"{self.directory}/presence_images"))
        for i in range(num_images):
            image = Image.open(f"{self.directory}/presence_images/image_{i}.png")
            self.presence_images.append(np.array(image))
        return np.array(self.presence_images)

    def get_absence_images(self):
        num_images = len(os.listdir(f"{self.directory}/absence_images"))
        for i in range(num_images):
            image = Image.open(f"{self.directory}/absence_images/image_{i}.png")
